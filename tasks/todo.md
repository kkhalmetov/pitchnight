...
- [ ] Dependencies и scripts из spec закреплены в `package.json` и lockfile.
- [ ] TypeScript strict включён; `/` рендерит минимальную семантическую страницу.
- [ ] Проект не требует env, базы или API-ключей для build.
...
- [ ] Layout имеет единый header/main/footer и корректную семантику.
- [ ] Design tokens поддерживают risk levels без зависимости только от цвета.
- [ ] Страница не имеет горизонтального скролла на 360 px и 1440 px.

**Verification:**

- [ ] `npm run build`
- [ ] Manual check: 360×800, 768×1024 и 1440×900.
- [ ] Manual check: видимый keyboard focus и `prefers-reduced-motion`.
...
- [ ] `lint`, `typecheck`, `test`, `test:coverage`, `test:e2e`, `build`, `verify`
  являются исполняемыми scripts.
- [ ] Unit smoke test и Playwright smoke test проходят на минимальном shell.
- [ ] `verify` останавливается при сбое любого обязательного шага.
...
- [ ] `npm run verify`
- [ ] Fresh-install build documented in commit notes.
- [ ] No secrets or untracked build output.
