...
- [x] `kk` и `ru` dictionaries имеют одинаковый типизированный набор ключей.
- [x] Initial locale: saved choice → browser language → `kk`.
- [x] Locale может меняться in-place, а выбранное значение переживает reload.

**Verification:**

- [x] `npm run test -- tests/i18n.test.ts`
- [x] `npm run typecheck`
- [x] Manual check: unsupported browser locale открывает `kk`.
...
- [x] Landing показывает четыре утверждённых факта с годом и рабочей ссылкой.
- [x] Весь текст, CTA, source labels и ARIA доступны на `kk` и `ru`.
- [x] Language switch меняет страницу без reload; CTA переводит к `#demo`.

**Verification:**

- [x] `npm run build`
- [x] Manual check: обе локали и все четыре source links.
- [x] Manual check: landing читаем на 360 px и projector viewport.
