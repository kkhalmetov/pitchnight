...
2. Главная цель MVP — убедительное трёхминутное демо, а не полная школьная ERP.
...
5. Интерфейс и питч — на русском; ключевые названия и короткие CTA допускают
   казахский. Полная локализация — после Pitch Night.
6. Презентация, сценарий демо и текст заявки входят в результат, потому что форма
   требует презентацию и ссылку на работающий продукт.

## Objective

Создать **TYNYS Mektep** — climate-safety copilot для школ жарких городов.
...
- mobile/desktop responsive UI, русский интерфейс;
- pitch deck, трёхминутный script, application copy и README.
...
components/                  UI grouped by demo/landing/data-source concerns
lib/environment/             Open-Meteo client, schema and fallback
lib/exposure/                index formula, levels and explanations
lib/schedule/                constraints and deterministic optimizer
data/                        dated fallback snapshot and demo schedule
tests/                       unit and integration tests
e2e/                         Playwright demo and responsive tests
docs/                        research, spec, methodology and pitch materials
...
- изменение основной аудитории со школ на другой сегмент;
- отправка формы Pitch Night от имени пользователя;
...
9. Подготовлены презентация, трёхминутный script, ответы для формы и резервный
   сценарий показа.
10. Production задеплоен на Vercel Hobby; commit SHA в GitHub соответствует
    задеплоенной версии.

## Open Questions

1. Подтверждает ли основатель концепцию **TYNYS Mektep** и название?
2. ФИО основателя/участников и размер команды потребуются только для
   презентации/готового текста формы; отправка формы не входит в автоматические
   действия без отдельного подтверждения.
3. Нужна ли казахская версия уже к финалу? По умолчанию MVP и deck будут на
   русском с казахским названием и короткими подписями.
